...
function SheetContent({
  ...
  // El ancho por defecto va como clase PLANA, no como `data-[side=…]:w-3/4`.
  // Con el prefijo de variante, tailwind-merge no lo reconoce como la misma
  // clave que el `w-full`/`sm:max-w-xl` del llamador: convivían los dos y en
  // CSS ganaba el selector de atributo por especificidad, así que el ancho
  // que pedía cada pantalla quedaba INERTE y sin ningún aviso. Calculado acá
  // en JS —`side` ya es un prop— cn() puede deduplicarlo y el llamador manda.
  // Sheets `top`/`bottom` no llevan ancho: los estira su `inset-x-0`.
  const anchoPorDefecto =
