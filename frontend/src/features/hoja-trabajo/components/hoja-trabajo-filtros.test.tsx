...
function makeOnBuscar() {
  const calls: HojaTrabajoFiltroValues[] = [];
  const fn = vi.fn((v: HojaTrabajoFiltroValues) => {
    calls.push(v);
  });
  return { fn, calls };
}
...
import { HojaTrabajoFiltros } from './hoja-trabajo-filtros';

function renderFiltros(onBuscar = vi.fn()) {
  capturedOnChange = null;
  return render(<HojaTrabajoFiltros onBuscar={onBuscar} />);
}
...
describe('HojaTrabajoFiltros — renderizado', () => {
  ...
  it('botón Consultar muestra "Consultando…" y está deshabilitado cuando isFetching=true', () => {
    render(<HojaTrabajoFiltros onBuscar={vi.fn()} isFetching />);
...
describe('HojaTrabajoFiltros — validación sin selección', () => {
...
describe('HojaTrabajoFiltros — consultar con rango válido', () => {
...
describe('HojaTrabajoFiltros — rango personalizado via capturedOnChange', () => {
