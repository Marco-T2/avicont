...
describe('LibroMayorFiltros — renderizado', () => {
  ...
  it('botón Consultar muestra "Consultando…" y está deshabilitado cuando isFetching=true', () => {
    setupMocks();
    render(<LibroMayorFiltros onBuscar={vi.fn()} isFetching />);
    const btn = screen.getByRole('button', { name: /consultando/i });
    expect(btn).toBeDisabled();
  });

  it('renderiza el toggle "Solo con movimiento" activado por defecto', () => {
    renderFiltros();
    const toggleSoloConMovimiento = screen.getByRole('switch', { name: /solo con movimiento/i });
    expect(toggleSoloConMovimiento).toBeChecked();
  });

  it('renderiza el toggle "Incluir anulados" desactivado por defecto', () => {
    renderFiltros();
    const toggleAnulados = screen.getByRole('switch', { name: /incluir anulados/i });
    expect(toggleAnulados).not.toBeChecked();
  });
});
...
describe('LibroMayorFiltros — validación sin selección', () => {
...
describe('LibroMayorFiltros — consultar con rango válido', () => {
  it('consultar tras emitir rango emite { fechaDesde, fechaHasta, incluirAnulados: false, soloConMovimiento: true }', async () => {
    const user = userEvent.setup();
  ...
  it('onBuscar emite incluirAnulados: true cuando el toggle está activo', async () => {
    ...
    await user.click(screen.getByTestId('mock-emitir-rango'));
    await user.click(screen.getByRole('switch', { name: /incluir anulados/i }));
    await user.click(screen.getByRole('button', { name: /consultar/i }));
  ...
  it('onBuscar emite soloConMovimiento: false cuando el toggle se desactiva', async () => {
    const user = userEvent.setup();
    const { fn: onBuscar, calls } = makeOnBuscar();
    renderFiltros(onBuscar);

    await user.click(screen.getByTestId('mock-emitir-rango'));
    await user.click(screen.getByRole('switch', { name: /solo con movimiento/i }));
    await user.click(screen.getByRole('button', { name: /consultar/i }));

    await waitFor(() => {
      expect(onBuscar).toHaveBeenCalled();
    });
    expect(calls[calls.length - 1]?.soloConMovimiento).toBe(false);
  });
...
describe('LibroMayorFiltros — rango personalizado via capturedOnChange', () => {
