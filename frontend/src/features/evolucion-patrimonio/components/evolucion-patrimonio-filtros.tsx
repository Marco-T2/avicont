...
// La exclusividad de modos la enforza handleSubmitInternal antes de construir
// el payload que se pasa a onBuscar.
// ============================================================
...
interface EvolucionPatrimonioFiltrosProps {
  /** Callback cuando el usuario envía el formulario con filtros válidos. */
  onBuscar: (values: EvolucionPatrimonioFiltroValues) => void;
  /** Indica si hay una búsqueda activa (para deshabilitar el botón). */
  isFetching?: boolean;
}

/**
 * Panel de filtros del Estado de Evolución del Patrimonio Neto.
 *
 * Modo "rango de fechas" por default (el reporte es anual: 01-ene a 31-dic de la
 * gestión). Modo "período fiscal" para evaluar un mes puntual. Toggle "Incluir
 * anulados" disponible en ambos modos.
 *
 * Los modos son mutuamente excluyentes; el payload que se pasa a onBuscar
 * contiene exactamente uno.
 */
export function EvolucionPatrimonioFiltros({
  onBuscar,
  isFetching = false,
}: EvolucionPatrimonioFiltrosProps): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        {modo === 'rango' && (
          <>
            <div className="space-y-1">
              <Label htmlFor="eepn-fecha-desde" className="text-xs text-muted-foreground">
                Desde
              </Label>
              <Input
                id="eepn-fecha-desde"
            ...
            <div className="space-y-1">
              <Label htmlFor="eepn-fecha-hasta" className="text-xs text-muted-foreground">
                Hasta
              </Label>
              <Input
                id="eepn-fecha-hasta"
        ...
        {modo === 'periodo' && (
          <div className="space-y-1">
            <Label htmlFor="eepn-periodo" className="text-xs text-muted-foreground">
              Período fiscal
            </Label>
            <Select
        ...
        <div className="flex items-center gap-2 pb-0.5">
          ...
          />
          <Label htmlFor="eepn-anulados" className="text-sm cursor-pointer">
            Incluir anulados
          </Label>
        </div>
