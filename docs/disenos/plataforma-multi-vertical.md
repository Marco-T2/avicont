...
conceptos** — "qué vertical es la org" y "está el módulo habilitado". Además, el
schema **no impide** que ambos sean `true` a la vez: la regla "un vertical por
org" es hoy **convención del flujo de creación, no invariante de base**. Esto es
exactamente el objeto de la decisión abierta §10.4.
...
| **`GET /me/permissions`** (permisos efectivos del usuario) | ❌ no existe |
| **Gating frontend** (`usePermissions` + filtrar sidebar/rutas/botones) | ❌ no existe |
| `NavItem` extensible (requiredPermission/modulo/pack) | ❌ no existe |
...
### 10.4 Exclusividad del vertical por org

El schema permite `contabilidadEnabled` y `granjaEnabled` ambos `true`; la regla
"un vertical por org" es solo convención del create. **Decisión**: ¿se fuerza un
vertical exclusivo por org (invariante de base), o una org puede ser
multi-vertical? **Recomendación**: vertical exclusivo por org; los packs cubren la
necesidad de "extender" sin volver multi-vertical. Confirmar antes de construir el
shell por vertical (§8).
...
2. **Shell/navegación por vertical (eje 1)** — cuando exista un segundo vertical
   que gatear. Requiere cerrar §10.4.
