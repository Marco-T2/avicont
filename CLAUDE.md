...
| Gate de `cerrar()` lee estado del cierre fuera de la `$transaction` | Deuda menor — matchea patrón preexistente del código (`cerrar()` no es una TX larga); no es F-03 real porque el gate solo lee y la escritura final (`voltear flag CERRADA`) es la TX. Si se endurecen los requisitos de serialización, pasar el objeto transaccional `tx` al gate. | Al próximo toque del módulo `cierre-ejercicio` o `gestiones-fiscales.service.ts` |
| Chequeos `generadoPorSistema` sin `=== true` explícito | Deuda cosmética — en JS/TS `if (x)` para un campo `boolean @default(false)` es equivalente, pero `=== true` documenta la intención. Aplicar al próximo toque. | Al próximo toque del módulo `comprobantes.service.ts` o `cierre-ejercicio` |
