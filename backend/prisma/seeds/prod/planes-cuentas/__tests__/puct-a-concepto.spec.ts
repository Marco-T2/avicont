...
//               tener su codigoPuct presente en MAPEO_PUCT_A_CONCEPTO.
// Invariante 2: toda entrada de MAPEO_PUCT_A_CONCEPTO debe estar presente
//               en la plantilla (CUENTAS_HOJA_COMERCIAL) con
//               `esRequeridaSistema: true`.
//
// Si este test falla:
//   - Agregaste una cuenta requerida sin mapear → agregar a MAPEO_PUCT_A_CONCEPTO.
//   - Agregaste un mapeo sin su cuenta en la plantilla → agregar la cuenta
//     a CUENTAS_HOJA_COMERCIAL con esRequeridaSistema: true.

import { CUENTAS_HOJA_COMERCIAL, MAPEO_PUCT_A_CONCEPTO } from '../comercial';

describe('plantilla COMERCIAL: coherencia esRequeridaSistema ↔ MAPEO_PUCT_A_CONCEPTO', () => {
  ...
  const codigosMapeados = new Set(Object.keys(MAPEO_PUCT_A_CONCEPTO));

  it('toda cuenta esRequeridaSistema está en MAPEO_PUCT_A_CONCEPTO', () => {
    const sinMapeo = [...codigosRequeridos].filter((c) => !codigosMapeados.has(c));
    expect(sinMapeo).toEqual([]);
  });

  it('toda entrada de MAPEO_PUCT_A_CONCEPTO corresponde a una cuenta esRequeridaSistema', () => {
  ...
  it('no hay conceptos duplicados en MAPEO_PUCT_A_CONCEPTO (cada concepto a una sola cuenta)', () => {
    const conceptos = Object.values(MAPEO_PUCT_A_CONCEPTO);
