...
describe('CustomRoleSlug', () => {
  describe('of — válidos', () => {
    it.each(['co', 'cobrador', 'cobrador-aux', 'granjero-turno-noche', 'rol-2', 'v2'])(
      'acepta "%s"',
      (raw) => {
  ...
  describe('of — inválidos', () => {
    ...
    it('rechaza tipos no-string', () => {
      ...
      expect(() => CustomRoleSlug.of(42 as unknown as string)).toThrow(CustomRoleSlugInvalidoError);
    });
  });

  describe('equals', () => {
    it('true si mismo valor', () => {
      expect(CustomRoleSlug.of('cobrador-aux').equals(CustomRoleSlug.of('cobrador-aux'))).toBe(
        true,
      );
    });

    it('false si distintos', () => {
      expect(CustomRoleSlug.of('cobrador').equals(CustomRoleSlug.of('contador'))).toBe(false);
    });
  });
});
