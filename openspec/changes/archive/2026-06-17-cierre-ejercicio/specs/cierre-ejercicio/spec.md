# Delta spec — cierre-ejercicio (capability nueva)

> Change: `cierre-ejercicio`
> Fecha: 2026-06-17
> Capability NUEVA. Requisitos `REQ-CE-*`.
> Inputs cerrados: `proposal.md`, `design.md`, sign-off engram
> `sdd/cierre-ejercicio/signoff` (decisiones A/B/C FIRMADAS por Marco).
...
3 están `CONTABILIZADO`. Las líneas se construyen con el algoritmo signed-net y
verifican partida doble en BOB (±Bs 0.01, §4.1) con `Money`. Referencia
regulatoria: Ley 843 art. 46 (cierre de cuentas de resultado y traslado a
patrimonio) + Código Tributario art. 47 (partida doble débito=crédito).
...
- **#1 Cerrar gastos y costos** — por cada cuenta hoja (`esDetalle=true`) clase
  ...
  `3.1.4.001 RESULTADO DE LA GESTIÓN` por `Σ|net|` de gastos. Invariante:
  `Σ haber (gastos) === debe (transitoria)`.
...
El `net` por cuenta hoja se calcula como
`net = (naturaleza === ACREEDORA) ? creditoBob − debitoBob : debitoBob − creditoBob`,
sobre saldos leídos con `excluirCierre=true` (REQ-CE-06). Las líneas se computan
en dominio puro con `Money`; cada comprobante verifica partida doble en BOB
(`Σdebe ≈ Σhaber`, ±Bs 0.01, §4.1) y lanza `CierrePartidaDobleError` si no cuadra.
...
GESTIÓN` (eliminada del seed y de orgs ya sembradas — ver delta `cuentas`/seed).
...
- **ENTONCES** responde 422 con código `CIERRE_EJERCICIO_SIN_MOVIMIENTO` (no hay
  nada que cerrar); no se genera comprobante alguno.
...
- **ENTONCES** los saldos base se leen con `excluirCierre=true`; las líneas
  recalculadas son idénticas a las de la primera generación (mismos saldos
  operativos), no acumulan los CIERRE previos.
...
mes en el año calendario del período `ordenEnGestion=12`; el comprobante se ata
al `periodoFiscalId` de esa fecha.
...
no al generar el borrador. El enforcement de no-editabilidad vive en el módulo
`comprobante` (ver delta `comprobante`).
...
- **ENTONCES** la constraint `@@unique` impide insertar dos veces el mismo slot;
  a lo sumo un set de cierres queda persistido.
...
gestión estén CERRADO y que el período `mesCierre` esté ABIERTO (los cierres se
contabilizan en el último período mientras sigue abierto, antes de cerrarlo). Si
la precondición no se cumple, responde 409 `CIERRE_EJERCICIO_PERIODO_NO_LISTO`.

> Nota (ambigüedad menor del design §7.1, paso 2): el design deja "a confirmar
> con spec" si se permite generar con el último período ya cerrado vía
> reapertura. Esta spec fija la precondición canónica (11 cerrados + mesCierre
> abierto); la corrección post-cierre usa el flujo de reapertura existente
> (REQ-CE-13), que vuelve a abrir el período y rehabilita la regeneración.

#### Escenario: período de cierre cerrado prematuramente
- **DADO** una gestión donde el período `mesCierre` ya está CERRADO (sin
  reapertura activa)
- **CUANDO** se invoca la generación
...
- **CUANDO** se invoca el cierre con un `id` que no existe o pertenece a otro
  tenant
- **ENTONCES** responde 404 `CIERRE_EJERCICIO_GESTION_NO_ENCONTRADA` (no
  distingue inexistente de ajena — defense in depth §4.2).
...
El sistema DEBE crear los (≤3) comprobantes de cierre en UNA transacción (los
que correspondan tras SKIP-on-zero, o ninguno). La contabilización es posterior y
se hace por comprobante (cada `contabilizar` su propia TX); un estado "1 de 3
contabilizado" es consistente y reversible (anular + regenerar, o contabilizar
los restantes). La gestión NO se cierra hasta que los comprobantes generados
estén todos CONTABILIZADO (ver delta `gestion-fiscal`).
...
permiso nuevo) y el módulo `contabilidad` habilitado.
...
- **DADO** un tenant con el módulo contabilidad deshabilitado (p. ej. vertical
  granja)
...
- **ENTONCES** los débitos/créditos de cada línea son strings de 2 decimales y
  `fechaContable` es `"YYYY-MM-DD"`.

### REQ-CE-16 — Regresión de cuadre de reportes (antes/después del cierre)

El sistema DEBE preservar, con los comprobantes CIERRE reales presentes, los 4
invariantes de cuadre (decisión C FIRMADA: NO se cambia el contrato `excluirCierre`
de reportes; solo se agrega regresión). Sea `R` el resultado del ejercicio de la
gestión `G` (derivado, excluyendo CIERRE) y `RA` el saldo de RESULTADOS
ACUMULADOS:

1. ER / EFE / Balance de Comprobación / Hoja de Trabajo de `G` (excluyen CIERRE)
   reportan `R` operativo **idéntico antes y después** de contabilizar los
   cierres.
2. BG de `G` a fecha mesCierre: la línea sintética "Resultado del Ejercicio" da
   ...
   (±Bs 0.01) — el resultado migró de la línea sintética a RA, no se duplicó.
...
Cada invariante DEBE verificarse para caso **utilidad** y caso **pérdida**.
...
- **ENTONCES** `patrimonioTotal` es idéntico (±Bs 0.01) en ambos snapshots; la
  línea sintética "Resultado del Ejercicio" pasa de `R` a ≈0 y RESULTADOS
  ACUMULADOS sube/baja en `R`/`|R|`.
...
- **ENTONCES** el EEPN incluye el traslado #3 en `otrosMovimientos` y cuadra con
  `saldoFinal`.
