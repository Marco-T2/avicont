# Delta spec — reportes: Balance de Comprobación de Sumas y Saldos

> Delta del capability `reportes`. Agrega el Balance de Comprobación.
> Requisitos `REQ-BC-*`. Naming alineado con `REQ-BG-*` (Balance General) y
> `REQ-ER-*` (Estado de Resultados) ya existentes.
...
El sistema DEBE ordenar las líneas por `codigoInterno` ASC
(`localeCompare`).
...
- `cuadra: boolean` = (`totalSumasDebito ≈ totalSumasCredito`) Y
  (`totalSaldoDeudor ≈ totalSaldoAcreedor`), tolerancia ±Bs 0.01 (§4.1).
...
`incluirAnulados=true` (§4.7). BORRADOR nunca se incluye (lo garantiza el port,
§4.1).
...
("700.00", §4.5) y las fechas como `"YYYY-MM-DD"` (§4.6).
