<!--
Change: comprobante-contacto-linea-ui
Fase: spec
Fecha: 2026-05-29
Status: EN PROGRESO
...
# Spec: Selector de Contacto por línea de comprobante (UI)

> Spec nueva — no existe una spec principal de `comprobante-linea-ui`.
> El backend ya valida `requiereContacto` y acepta `contactoId` en `LineaComprobanteDto`.
> Esta spec cubre exclusivamente el comportamiento UI observable.

---

## REQ-CCL-UI-01 — Selector de contacto en `linea-row.tsx`
...
pero el requerimiento visual se activa según REQ-CCL-UI-02.
...
## REQ-CCL-UI-02 — Validación visual cuando `requiereContacto = true`
...
## REQ-CCL-UI-03 — Preservar contacto en comprobante existente (mode=edit)
...
## REQ-CCL-UI-04 — Detalle read-only muestra nombre del contacto
...
## REQ-CCL-UI-05 — Manejo de errores del backend al contabilizar
...
## REQ-CCL-UI-06 — Accesibilidad del campo Contacto
...
## REQ-CCL-UI-07 — Tests de lógica y componentes
...
### 07.1 — `contacto-combobox.test.tsx`
...
### 07.2 — `linea-row.test.tsx` (extensión del test existente)
...
### 07.3 — Validación pre-contabilizar
...
|-----|--------|-----------|
| REQ-CCL-UI-01 | Selector de contacto en `linea-row.tsx` | 5 |
| REQ-CCL-UI-02 | Validación visual cuando `requiereContacto = true` | 4 |
| REQ-CCL-UI-03 | Preservar contacto en mode=edit | 2 |
| REQ-CCL-UI-04 | Detalle read-only muestra nombre | 3 |
| REQ-CCL-UI-05 | Manejo de errores backend al contabilizar | 3 |
| REQ-CCL-UI-06 | Accesibilidad del campo Contacto | 3 |
| REQ-CCL-UI-07 | Tests de lógica y componentes | Sub-escenarios en §07.1–07.3 |
...
|---|---|
| Columna Contacto siempre visible, requerida solo si `requiereContacto` | REQ-CCL-UI-01, REQ-CCL-UI-02 |
| Validación blanda (no bloquea borrador) + aviso antes de contabilizar | REQ-CCL-UI-02 |
| Enforcement duro queda en backend al contabilizar | REQ-CCL-UI-05 |
| Resolver nombre de contacto en read-only vía `contactoId` | REQ-CCL-UI-04 |
| Sin alta de contacto inline (directorio gestionado en `features/contactos`) | (fuera de scope) |
