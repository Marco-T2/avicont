...
- [ ] 2.1 **[RED unit]** Ampliar `backend/src/reportes/domain/libro-diario-errors.spec.ts`
...
- [ ] 2.2 **[GREEN]** Ampliar `backend/src/reportes/domain/libro-diario-errors.ts`
...
## Fase 3 — Backend Diario: DTO + Port

- [ ] 3.1 **[GREEN — forma]** Ampliar `backend/src/reportes/dto/libro-diario-query.dto.ts`:
  - Agregar campo:
    ```typescript
    @IsOptional()
    @IsUUID('4')
...
- [ ] 3.2 **[GREEN — forma]** Ampliar
  `backend/src/reportes/ports/comprobantes-reader.port.ts`:
  - Agregar `cuentaId?: string` a `LibroDiarioFiltros`:
    ```typescript
    ...
    ```
  - Agregar método al port abstracto `ComprobantesReaderPort` para el lookup de cuenta
    (el service necesita validar existencia + esDetalle):
    ```typescript
    /**
     * Busca una cuenta por id dentro del tenant.
     * Defense in depth (§4.2): filtra por organizationId en la query.
    ...
    abstract obtenerCuentaDetalle(
      tenantId: string,
      cuentaId: string,
    ): Promise<{ id: string; esDetalle: boolean } | null>;
    ```
  - JSDoc en el método (port es contrato público, §2.3 CLAUDE.md).
  > REQ-LD-12, REQ-LD-13, REQ-LD-14: contrato del adapter y del service.

  _Commit sugerido_: `feat(reportes): cuentaId en LibroDiarioQueryDto + ComprobantesReaderPort`
...
- [ ] 4.1 **[RED unit]** Ampliar `backend/src/reportes/libro-diario.service.spec.ts`
  — agregar describe anidado `'validación de cuenta (REQ-LD-12..16)'`:

  **Helpers adicionales necesarios**:
  - `makeComprobantesReaderMock()` ya existe — agregar `obtenerCuentaDetalle: jest.fn()`.
...
- [ ] 4.2 **[GREEN]** Modificar `backend/src/reportes/libro-diario.service.ts`:
  ...
  - Antes del paso 3 (tope defensivo), agregar paso 2.5:
    ...
    if (query.cuentaId !== undefined) {
      const cuenta = await this.comprobantesReader.obtenerCuentaDetalle(
        tenantId,
        query.cuentaId,
      );
...
- [ ] 5.1 **[RED integration]** Ampliar
  ...
  **Casos RED — `obtenerCuentaDetalle`** (nuevo método):
  - Cuenta de detalle del tenant → devuelve `{ id, esDetalle: true }`.
  - Cuenta agrupadora del tenant → devuelve `{ id, esDetalle: false }`.
  - UUID inexistente → devuelve `null`.
  - UUID pertenece a otro tenant → devuelve `null` (defense in depth §4.2).
  > REQ-LD-12, REQ-LD-13, REQ-LD-14, REQ-LD-15, REQ-LD-16.
...
- [ ] 5.2 **[GREEN]** Modificar
  ...
  **`obtenerCuentaDetalle` — nuevo método**:
  ```typescript
  async obtenerCuentaDetalle(
    tenantId: string,
    cuentaId: string,
  ): Promise<{ id: string; esDetalle: boolean } | null> {
    // Defense in depth (§4.2): filtramos por organizationId.
    // Una cuenta de otro tenant devuelve null — misma respuesta que inexistente.
  ...
  ```
  `findFirst` (no `findUnique`) para poder filtrar por dos campos sin asumir un índice compuesto único.
  Devuelve `null` si no existe o pertenece a otro tenant — defense in depth implícita.
  > REQ-LD-12, REQ-LD-13, REQ-LD-14, REQ-LD-15, REQ-LD-16.
...
- [ ] 6.1 **[GREEN — controller]** Modificar
...
- [ ] 6.2 **[RED e2e]** Ampliar `backend/test/libro-diario.e2e-spec.ts`
...
- [ ] 6.3 **[GREEN]** Hacer pasar todos los E2E del 6.2. Las implementaciones de Fases 4-6.1
...
- [ ] 6.4 **[verde PR B]** Suite completa verde desde `backend/`:
  ```bash
  DATABASE_URL="postgresql://postgres:postgres@127.0.0.1:5432/saas" \
    pnpm exec jest src/ --runInBand
  DATABASE_URL="postgresql://postgres:postgres@127.0.0.1:5432/saas" \
...
| 3 — DTO + Port Diario | 3.1–3.2 | GREEN (forma) | Fase 2 |
...
**Distribución**: 2 RED unit (2.1, 4.1) + 1 RED integration (5.1) + 1 RED e2e (6.2) + 4 RED vitest (1.1, 1.3, 7.3, 7.5) + el resto GREEN/verificación.
...
| `backend/src/reportes/dto/libro-diario-query.dto.ts` | Modificar — agregar `cuentaId?` | 3.1 |
| `backend/src/reportes/ports/comprobantes-reader.port.ts` | Modificar — `cuentaId?` en filtros + método `obtenerCuentaDetalle` | 3.2 |
| `backend/src/reportes/libro-diario.service.ts` | Modificar — agregar paso 2.5 de validación | 4.2 |
| `backend/src/reportes/libro-diario.service.spec.ts` | Modificar — agregar ~8 casos | 4.1 |
| `backend/src/reportes/adapters/prisma-comprobantes-reader.adapter.ts` | Modificar — `buildWhere` + `obtenerCuentaDetalle` | 5.2 |
| `backend/src/reportes/adapters/prisma-comprobantes-reader.adapter.integration.spec.ts` | Modificar — agregar ~10 casos | 5.1 |
...
Sin migración. Sin tocar `schema.prisma`. Sin tocar `app.module.ts` (el port nuevo es
método del port ya registrado — no agrega bindings nuevos). Sin tocar `reportes.module.ts`.
