...
- [ ] 1.1 Escribir `src/reportes/domain/balance-comprobacion-errors.spec.ts`:
  verifica `code`, `message` y clase base (`ValidationError`/`InvalidStateError`)
  de cada error (DR-5).
- [ ] 1.2 Implementar `src/reportes/domain/balance-comprobacion-errors.ts` con:
...
- [ ] 1.3 Verde: `pnpm exec jest src/reportes/domain/balance-comprobacion-errors.spec.ts`.

## 2. Builder de dominio (núcleo — cobertura ≥95%)

- [ ] 2.1 Definir los tipos internos `*Calculada` (Money) y el `BalanceComprobacionResult`
...
- [ ] 2.2 Escribir `src/reportes/domain/balance-comprobacion.spec.ts` cubriendo:
...
- [ ] 2.3 Implementar `src/reportes/domain/balance-comprobacion.ts`
...
- [ ] 3.1 Implementar `src/reportes/dto/balance-comprobacion-query.dto.ts`:
...
- [ ] 4.1 Escribir `src/reportes/dto/balance-comprobacion-response.dto.spec.ts`:
...
- [ ] 4.2 Completar `src/reportes/dto/balance-comprobacion-response.dto.ts`:
...
- [ ] 4.3 Verde: `pnpm exec jest src/reportes/dto/balance-comprobacion-response.dto.spec.ts`.

## 5. Service

- [ ] 5.1 Escribir `src/reportes/balance-comprobacion.service.spec.ts` con mock
...
- [ ] 5.2 Implementar `src/reportes/balance-comprobacion.service.ts`
  ...
  mapea a DTO).
- [ ] 5.3 Verde: `pnpm exec jest src/reportes/balance-comprobacion.service.spec.ts`.

## 6. Controller + module

- [ ] 6.1 Agregar `@Get('balance-comprobacion')` a
...
- [ ] 6.2 Registrar `BalanceComprobacionService` en `providers` de
...
- [ ] 7.1 Escribir `test/balance-comprobacion.e2e-spec.ts` (clon de
...
- [ ] 7.2 Verde E2E (Postgres arriba, ver §11.3):
...
- [ ] 8.1 Regenerar `backend/openapi.json` (`pnpm run openapi:dump` o equivalente).
- [ ] 8.2 Regenerar `frontend/src/types/api.generated.ts` (`pnpm run gen:api-types`
  en `frontend/`).
- [ ] 8.3 Commitear ambos artefactos junto al código (si no, el job CI
  `contract-drift` rompe el build).

## 9. Cierre de calidad

- [ ] 9.1 Typecheck: `pnpm exec tsc --noEmit -p tsconfig.json` (cero `any`, §2.5).
- [ ] 9.2 Lint: `pnpm run lint` (y `--fix` si aplica).
- [ ] 9.3 Regresión del módulo:
  `pnpm exec jest src/reportes/` + e2e de `reportes` (sin romper Balance General /
  Estado de Resultados existentes).
- [ ] 9.4 Verificar comentarios regulatorios presentes (§2.2: Código Tributario
