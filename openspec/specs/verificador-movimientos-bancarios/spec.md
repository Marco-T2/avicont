...
mostrarse la franja de saldos por cuenta con `fechaUltimoMovimiento`
SIEMPRE visible.

La marca de desactualización DEBE calcularse contra un **corte efectivo**
`min(hasta, hoy)` y solo DEBE encenderse cuando el atraso supera
`DIAS_TOLERANCIA_SALDO` (7 días). Cuando enciende, DEBE exhibir la
**magnitud** del atraso en días, no un badge pelado. Una cuenta sin
movimientos (`fechaUltimoMovimiento=null`) NO DEBE marcarse: tiene su
propio indicador.

> Nota: la primera versión marcaba con `fechaUltimoMovimiento < hasta` a
> secas y eso encendía la marca en TODAS las cuentas casi siempre — basta
> que el corte no caiga exacto en el día del último movimiento. Con las 3
> cuentas del entorno real marcadas de 3, la señal no informaba nada: un
> indicador siempre encendido es decoración, no advertencia. Dos motivos
> para las dos reglas: (1) un rango que termina en el futuro no puede
> exigir movimientos que todavía no ocurrieron; (2) una cuenta bancaria
> pasa días sin movimiento con total normalidad, y lo accionable es el
> atraso que sugiere extractos sin importar. Corregido post-archive del
> change de origen (PR #258); la spec del change archivado conserva la
> redacción original como registro histórico.
...
#### Scenario: Cross-cuenta oculta la columna saldo y marca el atraso con su magnitud

- GIVEN el modo cross-cuenta con una cuenta cuyo último movimiento es del
  2026-05-01 y un corte efectivo del 2026-07-25
- WHEN se renderiza la pantalla
- THEN la columna `saldo` de la tabla no se muestra
- AND esa cuenta exhibe su fecha con la marca de desactualización y los 85
  días de atraso

#### Scenario: Atraso dentro de la tolerancia — no se marca

- GIVEN una cuenta cuyo último movimiento es del 2026-07-21 y un corte
  efectivo del 2026-07-25
- WHEN se renderiza la franja de saldos
- THEN la cuenta muestra su `fechaUltimoMovimiento` SIN marca de
  desactualización: 4 días no superan la tolerancia

#### Scenario: Rango que termina en el futuro — el corte se acota a hoy

- GIVEN `hasta=2026-07-31`, hoy es 2026-07-25 y una cuenta con último
  movimiento del 2026-07-23
- WHEN se evalúa la desactualización
- THEN el atraso es de 2 días contra hoy, no de 8 contra `hasta`
- AND la cuenta no se marca
