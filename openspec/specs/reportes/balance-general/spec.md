...
El Balance aplica tres lógicas de dominio sobre el Mayor:
...
3. **Resultado del Ejercicio** en Patrimonio: calculado como
   ...
   corte, reutilizando el `BalanceReaderPort` que también usará el Estado de
   Resultados (Change 4).
...
El endpoint `GET /api/eeff/balance` requiere el parámetro `?fecha` en formato
`YYYY-MM-DD`. Si `fecha` está ausente o tiene formato inválido, el sistema responde
con HTTP 400 y código `REPORTES_BALANCE_FECHA_INVALIDA`.

El parámetro `fecha` es una `FechaContable` (calendario puro, sin hora ni zona
horaria). El sistema interpreta el corte como `fechaContable <= fecha`
(inclusive) al acumular movimientos.

Cuando la fecha pasa la validación de formato (`YYYY-MM-DD`) pero es
semánticamente inválida (ej. `2026-02-30`), el service rechaza con
...
#### Escenario: fecha semánticamente inválida — error 400 con code de dominio

- CUANDO se consulta `GET /api/eeff/balance?fecha=2026-02-30` (pasa el regex pero no existe en el calendario)
- ENTONCES el sistema responde HTTP 400 con `REPORTES_BALANCE_FECHA_INVALIDA`
  (el service rechaza con `FechaCorteInvalidaError`)

---

### REQ-BG-02: Inferencia de gestión vigente

El sistema determina automáticamente la **gestión fiscal vigente** para la
`fecha` de corte consultada, buscando la `GestionFiscal` cuyo rango contiene esa
fecha. Si ninguna gestión cubre la fecha indicada, el sistema responde HTTP 422
con código `REPORTES_BALANCE_SIN_GESTION`.

La gestión vigente delimita exclusivamente el período del **Resultado del Ejercicio**
(REQ-BG-09). Los Activos, Pasivos y el resto del Patrimonio se calculan con acumulado
histórico desde el primer movimiento hasta la fecha de corte, sin restricción por
gestión (REQ-BG-05).
...
El sistema incluye únicamente comprobantes con
`estado IN (CONTABILIZADO, BLOQUEADO)` al calcular todos los saldos del Balance.
El estado BORRADOR nunca aparece ni contribuye a ningún saldo,
...
`anulado = true` no contribuyen a ningún saldo del Balance (§4.7 CLAUDE.md).
Si `incluirAnulados=true`, el sistema los incluye en el cálculo.

El toggle corresponde a consulta de auditoría interna. La respuesta no incluye
marcador visual de anulados (es un reporte agregado, no listado de movimientos);
la semántica es "calcular saldos con/sin anulados".
...
El sistema calcula el **saldo neto** de cada cuenta hoja (`esDetalle=true`)
...
Empresa nueva sin asiento APERTURA → saldo inicial 0 (correcto).

El campo `esContraria` no modifica esta fórmula base. `esContraria` interviene
en la **propagación a grupos** (REQ-BG-06), no en el cálculo del saldo individual
de la hoja.

El helper compartido `reportes/domain/saldo-naturaleza.ts` implementa esta
fórmula; tanto el Libro Mayor como el Balance la usan para garantizar que ambos
reportes nunca diverjan (REQ-BG-16).
...
El sistema propaga los saldos de las cuentas hoja (`esDetalle=true`) hacia
...
El sistema no doble-cuenta saldos. Un agrupador que aparece como hijo de
otro agrupador no suma los saldos ya propagados desde sus propias hojas Y
...
el sistema **resta** su saldo neto del total de su grupo agrupador
en lugar de sumarlo.
...
### REQ-BG-08: Omisión de ramas vacías

El sistema poda el árbol antes de serializar: hojas con saldo 0, agrupadoras
sin ningún descendiente con saldo ≠ 0 y subsecciones sin contenido se omiten
de la respuesta. Solo las tres secciones raíz (Activo, Pasivo, Patrimonio) se
preservan siempre, aunque tengan total `"0.00"`.

Este comportamiento es estilo QuickBooks/Sage (§10.9 CLAUDE.md): el reporte
muestra lo que es relevante, no el árbol completo del plan de cuentas.
...
#### Escenario: subsección sin cuentas con saldo — omitida

- DADO una subsección `ACTIVO_NO_CORRIENTE` cuyas cuentas tienen todas saldo 0
- CUANDO se consulta el Balance General
- ENTONCES esa subsección no aparece dentro de `activo.subsecciones`
...
El sistema calcula el **Resultado del Ejercicio** como:
...
`[inicioGestion, fechaCorte]` (inicio de la gestión inferida hasta la fecha de
...
calculada (`esSintetica: true`, `cuentaId: null`). Su valor puede ser negativo
(pérdida del ejercicio).

<!-- Esta es la misma fuente de verdad que reutilizará el Estado de Resultados -->
<!-- (Change 4). Port de saldos compartido: garantiza que Balance y Estado de -->
<!-- Resultados nunca diverjan. -->
...
Ejercicio. El sistema trata ambos por separado (**tratamiento dual**).
...
La respuesta devuelve el balance como un árbol jerárquico con tres secciones
de primer nivel: `ACTIVO`, `PASIVO`, `PATRIMONIO`. Cada sección se divide en
...
contiene las cuentas con saldo que le correspondan.

La jerarquía dentro de cada subsección sigue `parentId`/`nivel` del plan de cuentas
del tenant. El orden dentro de cada nivel es por `codigoInterno` ASC.
...
El sistema verifica la ecuación contable `Activo = Pasivo + Patrimonio`
con tolerancia `±Bs 0.01` (§4.1 CLAUDE.md) y expone el resultado como
`cuadra: boolean` y `diferenciaBob: string` (diferencia en BOB) en
la raíz de la respuesta.

`cuadra=false` no es un error duro: el sistema devuelve HTTP 200 con el
...
El sistema filtra todos los movimientos, saldos y estructura de cuentas
por el `organizationId` del JWT activo (§4.2 CLAUDE.md). El adapter Prisma
incluye `lc.organizationId = $tenant` como predicado explícito en la
...
Ningún saldo ni cuenta de otro tenant aparece en el Balance,
...
El sistema protege `GET /api/eeff/balance` con el permiso
`contabilidad.eeff.read`. Un usuario sin ese permiso recibe HTTP 403.
...
devuelve HTTP 200 con un Balance en cero (todas las secciones con total
`"0.00"`, `cuadra: true`). No devuelve un error 404 ni 422 por ausencia
de plan de cuentas.
...
`cuentas[]` con las hojas con saldo ≠ 0 de esa subclase. El campo `nivel` informa
la profundidad de cada entrada en el árbol del plan de cuentas.
...
### REQ-BG-16: Helper compartido `saldo-naturaleza.ts`

El service del Balance usa el helper compartido de signo-por-naturaleza
(`reportes/domain/saldo-naturaleza.ts`) para calcular el saldo neto de cada hoja,
el mismo que usa el Libro Mayor. Este helper fue extraído desde
`libro-mayor.service.ts` como refactoring sin cambio de comportamiento,
cubierto por los tests existentes del Mayor.

La extracción garantiza que Balance y Mayor usen exactamente la misma fórmula
de naturaleza, previniendo divergencia entre ambos reportes.
...
- El Resultado del Ejercicio calculado es la misma cifra que aparecerá en el
  Estado de Resultados (Change 4). Al compartir el `BalanceReaderPort`,
  se garantiza que ambos reportes nunca diverjan — un requisito contable fundamental.
