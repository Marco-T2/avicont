# Gestión de packs en organización — Spec viva

<!--
Última edición: 2026-06-11
Última revisión contra core: 2026-06-11
Owner: backend-lead
Change de origen: packs-gestion-ui (PR #189, main)
-->

## Propósito

UI completa de gestión de packs sobre el riel construido en `packs-riel`.
Cubre dos niveles de gestión:

- **Nivel super-admin**: habilitar/revocar entitlements de packs a una org
  desde el panel `/platform-admin/orgs`.
- **Nivel Owner**: activar/desactivar complementos habilitados desde
  `/settings/complementos`.

Naming user-facing: **"Complementos"** (español). "Pack" = vocabulario interno
de código/schema.

Diseño completo: `docs/disenos/packs-eje2.md`. Riel base: `openspec/specs/packs-riel/spec.md`.

**Decisiones cerradas durante la implementación:**
- **Slice 0**: nuevo endpoint `GET /admin/platform/packs` (GAP-1) que expone
  `PackService.listarCatalogo`, gateado por `SuperAdminGuard`, con
  `@ApiOkResponse({ type: [PackResponseDto] })`. Tipos regenerados en
  `openapi.json` + `api.generated.ts`.
- **Fix C-01 (verificación adversarial)**: revocar pack usa `pack.id` (FK al
  catálogo) NO `entitlement.id` (PK del entitlement). El backend
  `DELETE /admin/platform/orgs/:id/packs/:packId` filtra por `packId` en
  `OrgPackEntitlement.packId`. Fix aplicado en `org-packs-sheet.tsx:180` y
  test correspondiente antes del merge.
- **`NavItem.requiredSystemRole?`** campo nuevo en la interfaz de navegación;
  `NavList` filtra con `useHasSystemRole(['OWNER','ADMIN'])`, fail-closed.
- **`RequireSystemRole`** nuevo componente de routing que protege rutas por
  SystemRole (clonado del patrón `RequirePermission`).
...
Revocar DEBE enviar el `pack.id` del catálogo (no el id del entitlement).
...
- THEN se llama `DELETE /admin/platform/orgs/:id/packs/:packId` con `packId = pack.id` (id del catálogo)
